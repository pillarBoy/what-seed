...
/* 遍历newConfig 对于config.template.js中有注释的条目，为之加上注释
 *  usage: copyExistingComments( path.resolve(__dirname, relativePath ) )
 */

module.exports = function copyExistingComments(newConfig, oldConfigPath){
  const srcFileContent = fs.readFileSync( oldConfigPath, 'utf-8' )

  const configFileArray = srcFileContent.split(/\r?\n/)
  const configFileContent = 'module.exports = ' + JSON.stringify(newConfig, (key, value, srcArray) => appendComments(key, value, configFileArray) ,2)
